...
export default function App() {
 ...
 const statsInFlight = useRef(false); const statsRefreshAllowed = useRef(true);
 async function loadStats() {
  if (statsInFlight.current) return;
  statsInFlight.current = true;
  try { const response = await fetch('/api/stats', {cache: 'no-store'}); if (!response.ok) throw Error(); setStats(await response.json()); setStatsError(false); statsRefreshAllowed.current = true; }
  catch { setStatsError(true); statsRefreshAllowed.current = false; }
  finally { statsInFlight.current = false; }
 }
 ...
 useEffect(() => { void loadStats(); void checkContract(); const timer = setInterval(() => { if (statsRefreshAllowed.current) void loadStats(); }, 60000); return () => { clearInterval(timer); cleanup.current(); }; }, []);
